...
bot.callbackQuery(/^refresh_/, async (ctx) => {
    ...
    // পূর্বের রিলিজ করা/অ্যাসেস করা নম্বরগুলো রিলিজ করা
    let dbData = readDb();
    const userId = ctx.from.id;
    ...
    dbData.numbers.forEach(n => {
        if (n.service === service && n.country === country && n.assigned_to === userId && n.status === "used") {
            n.assigned_to = null;
            n.status = "available"; // Release assigned number
        }
    });
    writeDb(dbData);

    await deliverNumbers(ctx, service, country);
    ctx.answerCallbackQuery({ text: "Fresh number provided!" }).catch(()=>{});
});
...
async function deliverNumbers(ctx, service, country) {
    ...
    const selectedNumbers = availableNumbers.slice(0, qty);
    ...
    selectedNumbers.forEach((numObj) => {
        ...
        // Dynamic Copy Text Button format
        actionKeyboard.copyText(info.fullNumberWithPlus, info.fullNumberWithPlus).row();
    ...
    // Dynamic prefix without plus for remove prefix button
    const firstNumClean = selectedNumbers[0].phone_number.replace(/[^\d]/g, "");
    const firstInfo = parsePhoneNumberInfo(firstNumClean);
    const noPrefixNum = firstInfo.nationalNumber;

    actionKeyboard.copyText(`✂️ Remove Prefix (${noPrefixNum})`, noPrefixNum).row();
    ...
    try {
        if (ctx.callbackQuery) {
            await ctx.editMessageText(deliveredText, { reply_markup: actionKeyboard, parse_mode: "HTML" });
        } else {
            await ctx.reply(deliveredText, { reply_markup: actionKeyboard, parse_mode: "HTML" });
        }
    } catch (err) {
        await ctx.reply(deliveredText, { reply_markup: actionKeyboard, parse_mode: "HTML" }).catch(()=>{});
    }
}
