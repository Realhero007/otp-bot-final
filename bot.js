...
bot.callbackQuery(/^getnum_/, async (ctx) => {
    ...
    await deliverNumbers(ctx, service, country, true); // true = with prefix
    ctx.answerCallbackQuery().catch(()=>{});
});
...
bot.callbackQuery(/^refresh_/, async (ctx) => {
    ...
    await deliverNumbers(ctx, service, country, true);
    ctx.answerCallbackQuery({ text: "Fresh number updated!" }).catch(()=>{});
});
...
bot.callbackQuery(/^toggleprefix_/, async (ctx) => {
    ...
    // Toggle: if currently with prefix, show without prefix (false) and vice-versa
    await deliverNumbers(ctx, service, country, !currentHasPrefix);
    ctx.answerCallbackQuery().catch(()=>{});
});

async function deliverNumbers(ctx, service, country, showWithPrefix = true) {
    const dbData = readDb();
    const userId = ctx.from.id;
    const qty = dbData.numberQty || 1;

    // Find assigned number for this user/service or fallback to available
    let selectedNumbers = dbData.numbers.filter(
        n => n.service === service && 
        n.country === country && 
