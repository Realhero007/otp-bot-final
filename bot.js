...
function readDb() {
    if (!fs.existsSync(DB_FILE)) {
        ...
        fs.writeFileSync(DB_FILE, JSON.stringify({ 
            ...
            apiUrls: defaultApis,
            checkDelay: 4000,
            numberQty: 1, // Default 1 ta number dibe
...
async function deliverNumbers(ctx, service, country) {
    ...
    // স্টক থেকে নির্দিষ্ট সংখ্যক নম্বর ফিল্টার করা
    ...
    // এডমিন যত সেট করেছে বা স্টকে যতগুলো খালি আছে ততটি নম্বর নেওয়া
    ...
    deliveredText += `✨ <b>Your Requested Number(s):</b>\n\n`;
    ...
    selectedNumbers.forEach((numObj) => {
        ...
        // এক ক্লিকে টেক্সট কপি করার মতো ফরম্যাট
        deliveredText += `<code>${info.fullNumberWithPlus}</code>\n`;

        // লাইভ ওটিপি চেকার অন করা
        startLiveOtpCheck(numObj.id, info.fullNumberWithPlus, service, userId, numObj.country);
    });

    writeDb(dbData);

    deliveredText += `\n⏳ <i>Waiting for OTP... (নম্বরের ওপর ক্লিক করলেই কপি হবে)</i>`;

    const actionKeyboard = new InlineKeyboard()
        .text("🌐 Change Country", `user_${service}`).row()
        .url("🔑 Get OTP Group ↗️", OTP_GROUP_LINK);
