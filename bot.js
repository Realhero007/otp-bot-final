...
function getCountryFlag(countryName) {
    ...
    if (isoCode && isoCode.length === 2 && isoCode !== "XX") {
...
// 🎯 UPDATED: Universal OTP & Security Code Extractor (Links + AlphaNumeric Codes)
function extractOtp(messageText) {
    if (!messageText || typeof messageText !== 'string') return null;
    
    // 1. Check for URL/Link
    const linkMatch = messageText.match(/(https?:\/\/[^\s]+)/gi);
    if (linkMatch) return linkMatch[0];

    // 2. Check for explicit keywords like "is DN7UDX", "code 12345", etc.
    const keywordMatch = messageText.match(/(?:code|is|pin|otp)\s*(?:is|:)?\s*([a-z0-9]{4,8})\b/i);
    if (keywordMatch && keywordMatch[1]) return keywordMatch[1];

    // 3. Check for standalone Alphanumeric/Numeric codes (4 to 8 characters)
    const generalMatch = messageText.match(/\b[A-Z0-9]{4,8}\b/i);
...
async function showCountriesList(ctx, serviceName) {
    ...
    countries.forEach((country, index) => {
        ...
        const price = dbData.serviceRates[rateKey] || dbData.serviceRates[serviceName] || 0.00;

        const buttonLabel = `${flag} ${country} [${iso}] - ৳${price} BDT (${counts[country]})`;
        keyboard.text(buttonLabel, `getnum_${serviceName}_${encodeURIComponent(country)}`).row();
    });

    const msg = `🌍 <b>${serviceName}</b> সার্ভিসের জন্য কান্ট্রি নির্বাচন করুন:`;
...
bot.callbackQuery(/^getnum_/, async (ctx) => {
    ...
    await deliverNumbers(ctx, service, country, false);
    ctx.answerCallbackQuery().catch(()=>{});
});

// 🎯 UPDATED: Prefix Toggle Handler
bot.callbackQuery(/^toggleprefix_/, async (ctx) => {
    const parts = ctx.callbackQuery.data.split("_");
    const service = parts[1];
    const country = decodeURIComponent(parts[2]);
    const hidePrefix = parts[3] === "1"; 
    
    await deliverNumbers(ctx, service, country, hidePrefix);
    ctx.answerCallbackQuery().catch(()=>{});
});

// 🎯 MULTI-NUMBER DELIVERY ENGINE (UPDATED: Clean Copy Text & Prefix Removal)
async function deliverNumbers(ctx, service, country, hidePrefix = false) {
    let dbData = readDb();
    const userId = ctx.from.id;
    ...
    activeUserNumbers.forEach((numObj, index) => {
        ...
        // Hide Prefix logic based on user toggle
        const displayNum = hidePrefix ? info.nationalNumber : info.fullNumberWithPlus;

        // 🎯 CLEAN COPY TEXT BUTTON (No "Copy Num #1" text)
        actionKeyboard.copyText(`${displayNum}`, `${displayNum}`).row();
    ...
    // Toggle prefix button label
    const toggleLabel = hidePrefix ? "➕ Add Country Code" : "✂️ Remove Country Code";
    const nextToggleState = hidePrefix ? "0" : "1";

    actionKeyboard.text(toggleLabel, `toggleprefix_${service}_${encodeURIComponent(country)}_${nextToggleState}`).row()
                  .text("🔄 Refresh (Clear Used & Get New)", `user_${service}`).row()
                        ...
                        if (activation && rawMessage) {
                            ...
                            if (otpCode) {
                                ...
                                if (!userSentOtps.has(uniqueKey)) {
                                    ...
                                    const otpReward = dbData.serviceRates[rateKey] || dbData.serviceRates[serviceName] || 0.00;
...
bot.hears("📱 Add Number", async (ctx) => {
    ...
    const services = dbData.services || [];
    if (services.length === 0) {
        await ctx.reply("❌ কোনো অ্যাক্টিভ সার্ভিস পাওয়া যায়নি। আগে সার্ভিস যোগ করুন।").catch(()=>{});
        return;
    }

    const keyboard = new InlineKeyboard();
    services.forEach((srv, idx) => {
        keyboard.text(`📱 ${srv}`, `addnum_srv_${srv}`);
        if (idx % 2 !== 0) keyboard.row();
    });

    await ctx.reply("📱 <b>Step 1:</b> যে সার্ভিসে নম্বর অ্যাড করতে চান সেটি সিলেক্ট করুন:", {
        reply_markup: keyboard,
        parse_mode: "HTML"
    }).catch(()=>{});
});

bot.callbackQuery(/^addnum_srv_/, async (ctx) => {
    const serviceName = ctx.callbackQuery.data.replace("addnum_srv_", "");
    adminState[ctx.from.id] = { 
        step: "awaiting_addnum_country", 
        service: serviceName 
    };

    await ctx.editMessageText(`🛠️ <b>Selected Service:</b> ${serviceName}\n\n🌍 <b>Step 2:</b> কান্ট্রি নেম লিখে পাঠান (যেমন: Bangladesh, India, USA):`, {
        parse_mode: "HTML"
    }).catch(()=>{});
    ctx.answerCallbackQuery().catch(()=>{});
});
...
bot.on("message:text", async (ctx) => {
    ...
    if (state.step === "awaiting_addnum_country") {
        const countryInput = ctx.message.text.trim();
        adminState[userId].country = countryInput;
        adminState[userId].step = "awaiting_addnum_rate";

        await ctx.reply(`🛠️ <b>Service:</b> ${state.service}\n🌍 <b>Country:</b> ${countryInput}\n\n💵 <b>Step 3:</b> এই সার্ভিস ও কান্ট্রির জন্য প্রতি ওটিপি রেট (BDT) কত হবে লিখুন:\n(যেমন: 0.5 বা 5.00)`, {
            parse_mode: "HTML"
        }).catch(()=>{});

    } else if (state.step === "awaiting_addnum_rate") {
        const rateInput = parseFloat(ctx.message.text.trim());
        if (isNaN(rateInput)) {
            await ctx.reply("❌ অনুগ্রহ করে সঠিক রেট (সংখ্যায়) দিন: (যেমন: 0.5 বা 5.00)").catch(()=>{});
            return;
        }

        adminState[userId].rate = rateInput;
        adminState[userId].step = "awaiting_addnum_list";

        // Save Rate
        const rateKey = `${state.service}_${state.country}`;
        if (!dbData.serviceRates) dbData.serviceRates = {};
        dbData.serviceRates[rateKey] = rateInput;
        writeDb(dbData);

        await ctx.reply(`🛠️ <b>Service:</b> ${state.service}\n🌍 <b>Country:</b> ${state.country}\n💵 <b>Rate:</b> ৳${rateInput} BDT\n\n📱 <b>Step 4:</b> এবার নম্বরগুলো সিরিয়ালে নিচে নিচে (Line by line) লিখে পাঠান:\n\nউদাহরণ:\n<code>01800000000\n01870000000</code>`, {
            parse_mode: "HTML"
        }).catch(()=>{});

    } else if (state.step === "awaiting_addnum_list") {
        ...
        await ctx.reply(`✅ <b>সফলভাবে ${addedCount} টি নম্বর যুক্ত করা হয়েছে!</b>\n\n🛠️ সার্ভিস: ${state.service}\n🌍 দেশ: ${state.country}\n💵 রেট: ৳${state.rate} BDT`, { parse_mode: "HTML" }).catch(()=>{});

    } else if (state.step === "awaiting_number_limit") {
