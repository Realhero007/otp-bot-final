...
function readDb() {
    ...
    try {
        ...
        if (!data.sentOtps) data.sentOtps = [];
        ...
        if (!data.customOtpMsg) data.customOtpMsg = "🚀 **FAST OTP RECEIVED!**\n\n📱 `{service}` | {flag} `{country}` | 📞 `+{phone}`\n\n💬 *\"{message}\"*\n\n👉 `{code}` *(Tap to Copy)*\n\n💰 **+৳{reward} BDT added to your balance!**";
        return data;
    } catch (e) {
        return { numbers: [], admins: [MASTER_ADMIN], users: [], services: [], serviceRates: {}, balances: {}, withdrawals: [], sentOtps: [], apiUrls: [], checkDelay: 4000, numberQty: 1 };
    }
}
...
// --- Country Delivery Core Logic (Requested Formatted Button Layout) ---
async function showCountriesList(ctx, serviceName) {
    ...
    countries.forEach((country, index) => {
        ...
        // 🔹 ২ ডিজিটের ISO কোড এক্সট্র্যাক্ট করা (যেমন: Tunisia (TN) থেকে TN বের করা)
        const match = country.match(/\(([A-Z]{2})\)/);
        const isoCode = match ? match[1] : "🌐";
        
        // 🔹 দেশের নাম ফরম্যাট করা
...
// --- Fixed Refresh Callback Handler ---
...
async function deliverNumbers(ctx, service, country, showWithPrefix = true, isRefresh = false, excludedIds = new Set()) {
    ...
    if (selectedNumbers.length === 0) {
        ...
        if (availableNumbers.length === 0) {
            const failText = `❌ দুঃখিত, এই মুহূর্তে \({service} (\){country}) সার্ভিসের কোনো নতুন নম্বর খালি নেই।`;
            try {
